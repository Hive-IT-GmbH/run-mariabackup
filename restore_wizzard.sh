...
if [ "$answer" = "y" ]; then
    ...
    echo ""
    echo "2. Discard Tablespace"
    echo ""
    echo "   ALTER TABLE ${database_name}.${table_name_backup} DISCARD TABLESPACE;"
    echo ""
    echo "3. Copy the the IBD and CFG files for the table from the backup directory to the data directory of the database."
    ...
    echo ""
    echo "4. Modify Ownership of the files"
    echo ""
    echo "   chown mysql:mysql $MYSQL_DATA_DIR/$database_name/$table_name_backup.{ibd,cfg}"
    echo ""
    echo "5. Re-Import the Tablespace again"
    echo ""
    echo "   ALTER TABLE ${database_name}.${table_name_backup} IMPORT TABLESPACE;"
    echo ""
